#!/usr/bin/env python3
"""
log_scanner_no_source.py

This script scans log files to find those containing the "No sources found" error message
...
def main():
    parser = argparse.ArgumentParser(
        description='Scan log files for "No sources found" error message and move them.'
    )
    parser.add_argument(
        '-d', '--directory', 
    ...
    parser.add_argument(
        '-o', '--output', 
        default='no_source_error_report.txt',
        help='Output file to save report (default: no_source_error_report.txt)'
    )
    parser.add_argument(
        '-m', '--move-to', 
        default='./FlowDroid_output_minor/error/no_source_error',
        help='Directory to move files containing the error'
    )
    parser.add_argument(
    ...
    error_message = "[main] ERROR soot.jimple.infoflow.android.SetupApplication$InPlaceInfoflow - No sources found, aborting analysis"
    ...
    print(f"Scanning for files containing the 'No sources found' error message...")
    matching_files = scan_for_error_files(search_path, error_message, args.pattern)
    print(f"Found {len(matching_files)} files containing the error message.")
    
    # Write results to the output file
    with open(args.output, 'w', encoding='utf-8') as output_file:
        # Write header with summary
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        output_file.write(f"No Sources Found Error Report - {timestamp}\n")
        output_file.write(f"Error message: {error_message}\n\n")
        output_file.write(f"SUMMARY\n")
        output_file.write(f"-------\n")
        output_file.write(f"Total files with 'No sources found' error: {len(matching_files)}\n\n")
        
        # Write the list of files
        output_file.write(f"LIST OF FILES CONTAINING THE ERROR\n")
        output_file.write(f"--------------------------------\n")
        ...
        if matching_files:
            ...
            output_file.write("No files containing the error message were found.\n")
