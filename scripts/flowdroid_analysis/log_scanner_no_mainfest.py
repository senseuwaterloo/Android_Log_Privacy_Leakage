#!/usr/bin/env python3
"""
Script to move log files that contain the specific zip error message
...
def get_files_with_error(search_path, error_message, file_pattern=None):
    """
    Get files that contain the error message.
    ...
    for root, _, files in os.walk(search_path):
        for filename in files:
            ...
            # Only include files that have the error
...
def main():
    parser = argparse.ArgumentParser(description='Move log files with the zip error message.')
    ...
    parser.add_argument('-o', '--output', default='zip_error_files_report.txt',
                        help='Output file to save report (default: zip_error_files_report.txt)')
    parser.add_argument('-d', '--destination', 
                        default='./zip_error_files',
                        help='Destination directory for moving files with error')
    ...
    error_message = "java.lang.RuntimeException: Error when looking for manifest in apk: java.util.zip.ZipException: error in opening zip file"
    ...
    # Find all files that contain the error
    print(f"Scanning for log files that contain the zip error message...")
    files_with_error = get_files_with_error(source_path, error_message, args.pattern)
    print(f"Found {len(files_with_error)} files with the zip error message.")
    
    # Write results to the output file
    with open(args.output, 'w', encoding='utf-8') as output_file:
        # Write header with summary
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        output_file.write(f"Zip Error Log Files Report - {timestamp}\n")
        output_file.write(f"Files that contain the error: {error_message}\n\n")
        output_file.write(f"SUMMARY\n")
        output_file.write(f"-------\n")
        output_file.write(f"Total files with zip error: {len(files_with_error)}\n\n")
        
        # Write the list of files
        output_file.write(f"LIST OF FILES WITH ZIP ERROR\n")
        output_file.write(f"---------------------------\n")
        ...
        if files_with_error:
            ...
            output_file.write("No files with the zip error were found.\n")
